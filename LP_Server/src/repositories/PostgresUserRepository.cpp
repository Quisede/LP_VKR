...
PostgresUserRepository::PostgresUserRepository(PostgresConnection& conn) : connection(conn) {
    ...
    PGresult* res = PQexec(
        ...
        "SELECT DISTINCT group_name FROM users WHERE role = 'Student' AND group_name <> '' "
...
User PostgresUserRepository::createUser(
    ...
    const std::string normalizedFirstName = firstName.empty() ? login : firstName;
    const char* params[] = {
        login.c_str(),
        passwordHash.c_str(),
        roleStr.c_str(),
        normalizedFirstName.c_str(),
        lastName.c_str(),
        groupName.c_str(),
        email.c_str(),
        phone.c_str()
    ...
    PGresult* res = PQexecParams(
        connection.get(),
        "INSERT INTO users(login, password_hash, role, first_name, last_name, group_name, email, phone) "
...
std::vector<CourseStudent> PostgresUserRepository::getStudentsForCourse(int courseId) {
    ...
    PGresult* res = PQexecParams(
        connection.get(),
        "SELECT u.id, u.login, u.group_name, "
        ...
        "GROUP BY u.id, u.login, u.group_name "
        "ORDER BY u.group_name, u.id",
        1,
        nullptr,
        params,
        nullptr,
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        student.groupName = PQgetvalue(res, i, 2);
        student.testProgress = std::stoi(PQgetvalue(res, i, 3));
        student.lessonProgress = std::stoi(PQgetvalue(res, i, 4));
...
std::vector<std::string> PostgresUserRepository::getGroups() {
    std::lock_guard<std::mutex> lock(connection.mutex());

    PGresult* res = PQexec(
        connection.get(),
        "SELECT name FROM groups WHERE name NOT IN ("
        "  SELECT DISTINCT group_name FROM users WHERE role <> 'Student' AND group_name <> ''"
        ") "
        "UNION "
        "SELECT DISTINCT group_name AS name FROM users WHERE role = 'Student' AND group_name <> '' "
        "ORDER BY name");

    if (PQresultStatus(res) != PGRES_TUPLES_OK) {
        std::string error = PQerrorMessage(connection.get());
        PQclear(res);
        throw std::runtime_error("Failed to get groups: " + error);
    }
...
std::string PostgresUserRepository::createGroup(const std::string& groupName) {
    std::lock_guard<std::mutex> lock(connection.mutex());

    const char* params[] = {groupName.c_str()};
    PGresult* res = PQexecParams(
        connection.get(),
    ...
    if (PQresultStatus(res) != PGRES_TUPLES_OK || PQntuples(res) == 0) {
        std::string error = PQerrorMessage(connection.get());
        PQclear(res);
        throw std::runtime_error("Failed to create group: " + error);
    }
...
std::string PostgresUserRepository::renameGroup(const std::string& oldName, const std::string& newName) {
    std::lock_guard<std::mutex> lock(connection.mutex());

    const char* params[] = {oldName.c_str(), newName.c_str()};
    const char* newParam[] = {newName.c_str()};
    PGresult* res = PQexecParams(
        connection.get(),
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        std::string error = PQerrorMessage(connection.get());
        PQclear(res);
        throw std::runtime_error("Failed to rename group: " + error);
    }
    PQclear(res);

    res = PQexecParams(
        connection.get(),
        "UPDATE users SET group_name = $2 WHERE role = 'Student' AND group_name = $1",
        2,
        nullptr,
        params,
        nullptr,
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        std::string error = PQerrorMessage(connection.get());
        PQclear(res);
        throw std::runtime_error("Failed to update students group: " + error);
    }
    PQclear(res);

    const char* oldParam[] = {oldName.c_str()};
    res = PQexecParams(
        connection.get(),
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        std::string error = PQerrorMessage(connection.get());
        PQclear(res);
        throw std::runtime_error("Failed to remove old group: " + error);
    }
    PQclear(res);
    return newName;
}

void PostgresUserRepository::deleteGroup(const std::string& groupName) {
    std::lock_guard<std::mutex> lock(connection.mutex());

    const char* params[] = {groupName.c_str()};
    PGresult* res = PQexecParams(
        connection.get(),
        "UPDATE users SET group_name = '' WHERE role = 'Student' AND group_name = $1",
        1,
        nullptr,
        params,
        nullptr,
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK) {
        std::string error = PQerrorMessage(connection.get());
        PQclear(res);
        throw std::runtime_error("Failed to delete group: " + error);
    }
    PQclear(res);
